...
async function checkCoachTrainer() {
  try {
    ...
    console.log(`Found ${approvedTrainers.length} records in approvedTrainers table:`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        const coachTrainer = data.trainers?.find(t => 
          t.name?.includes('COACH') || t.email === 'coach@gmail.com'
        );
