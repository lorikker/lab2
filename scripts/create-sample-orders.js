...
// Run with: node scripts/create-sample-orders.js user@example.com
...
async function createSampleOrders() {
  ...
  if (!email) {
    ...
    console.error(
      "Usage: node scripts/create-sample-orders.js user@example.com",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      console.log(
        `Created order #${orderNumber} with ${numItems} items, total: $${total.toFixed(2)}`,
      );
    }
